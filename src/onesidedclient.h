...
inline void OneSidedClient::read_async(uint32_t offset, uint32_t size) {
  ...
  uintptr_t remote_addr =
      reinterpret_cast<uintptr_t>(get_remote_base_addr()) + offset;
  uintptr_t local_addr =
      reinterpret_cast<uintptr_t>(get_local_base_addr()) + offset;
...
inline HostMemoryAsyncRead
OneSidedClient::readfromcoro(uint32_t offset, uint32_t size) noexcept {
  assert(onesready);
...
inline void *OneSidedClient::get_remote_base_addr() { return host_mr.addr; }

inline void *OneSidedClient::get_local_base_addr() { return rdma_mr->addr; }
