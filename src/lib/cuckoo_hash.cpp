...
struct cuckoo_hash_item *cuckoo_hash_lookup(const struct cuckoo_hash *hash,
                                            const void *key, size_t key_len) {
  uint32_t h1, h2;
...
void cuckoo_hash_remove(struct cuckoo_hash *hash,
  ...
  if (hash_item) {
    struct _cuckoo_hash_elem *elem =
        ((struct _cuckoo_hash_elem *)((char *)hash_item -
                                      offsetof(struct _cuckoo_hash_elem,
                                               hash_item)));
...
struct cuckoo_hash_item *cuckoo_hash_insert(struct cuckoo_hash *hash,
                                            const void *key, size_t key_len,
                                            void *value) {
  uint32_t h1, h2;
  compute_hash(key, key_len, &h1, &h2);
  ...
  while (elem != end) {
    if (elem->hash1 != elem->hash2) {
      ...
      */
      ...
      if (bin <= elem && elem < bin + hash->bin_size) return &elem->hash_item;
    }
